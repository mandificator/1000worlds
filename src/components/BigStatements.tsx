...
const statements = [
  ...
  },
  {
    ...
    title: "burn to mint",
    body: [
      "A uniform-price auction in $WORLDS.",
      "One price for everyone — every token spent is burned.",
    ],
    href: "/whitelist",
    ...
    <>
      {statements.map((s) => (
        ...
        >
          ...
          <p className="max-w-2xl text-lg text-parchment-dim sm:text-2xl">
            {s.body.map((line, i) => (
              <span key={line}>
                {i > 0 && <br />}
                {line}
